...
def test__extract_tags_3():
    assert HTML_Validator._extract_tags('<strong></strong>') ==\
            ['<strong>', '</strong>']


def test__extract_tags_4():
    assert HTML_Validator._extract_tags('python in <strong>fun</strong>') ==\
            ['<strong>', '</strong>']


def test__extract_tags_5():
    assert HTML_Validator._extract_tags('<a><b><c></a></b><f>') ==\
            ['<a>', '<b>', '<c>', '</a>', '</b>', '<f>']
